...
let archive = (destination, files, cwd, force, callback) => {
    ...
    let normaliseCwd = path.normalize(cwd) + path.sep;

    // Remove the root folder path because it is redundant and require for 7z module and add "
    // around the path to accept space in file path.
    files = files.map((el) => {
        return `"${el.replace(normaliseCwd, "")}"`;
    });
