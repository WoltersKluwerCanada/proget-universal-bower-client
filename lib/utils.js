 ...
 */
function parseIgnoreFile(filePath, omittedFiles) {
...
function getNpmRc(callback) {
    ...
    if (process.env.NODE_ENV !== "test") {
        npmRcFilePath = path.join(os.homedir(), ".npmrc");
    } else {
        npmRcFilePath = path.join(__dirname, "..", "test/data/.npmrc");
    }
    ...
    getNpmRc((err, data) => {
        ...
        if (err) {
            ...
            try {
                ...
                let usr = regex.npmUsername.line.exec(data).toString().replace(regex.npmUsername.header, "").replace(/"/g, "");
